  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 py-12 px-6">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }}
          ...
          <div className="grid grid-cols-2 gap-4">
            {barbeiros.length === 0 ? <p>Nenhum barbeiro disponível</p> :
              barbeiros.map((b) => (
                <div key={b.id} onClick={() => setBarbeiroSelecionado(b.id)}
                  ...
                  {b.foto && <img src={b.foto} alt={b.nome} className="w-20 h-20 rounded-full mx-auto mb-3 object-cover" />}
      ...
      <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }}
        className="max-w-4xl mx-auto bg-white shadow-xl rounded-2xl p-8 mt-12">
        <h2 className="text-2xl font-semibold mb-6 text-gray-800 text-center">Escolha o horário</h2>
        <div className="grid grid-cols-3 gap-4">
          {horarios.map((hora) => (
            <button key={hora} onClick={() => setHoraSelecionada(hora)}
